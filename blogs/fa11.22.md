...
$$
According to lemma 2.1, $\exist f_0\in X'$, subject to 
$$
...
Method one:
 ...
 We know it is continuous at 0., that is for any $\epsilon>0$ ,$\exist\ \delta>0$ subject to all point $x\in X$ subject to $||x||<\delta$, 
$$
p(x)<\epsilon.
$$
...
It is bounded at zero, that is $\exist\ \delta>0,\forall\ ||x||\leq \delta$,
$$
p(x)\leq 1.
$$
...
$$
||y_N(t)||_1=\int^b_a\ \chi_{E_N\cap E}\cdot sgn\ y(t)\ dt=m({E_N\cap E})
$$
...
Now that $T$ is an bijection, we should prove for $\forall x',\ \exist\ y',\ s.t.\ T'y'=x'$. Difine 
$$
y'(y)=x'(x).
$$
...
## 22.Prove $(1) \ \ ^0N(A')=\overline{R(A)}.      $    $(2)\quad\overline{R(A')}\subset N(A)^0$.
